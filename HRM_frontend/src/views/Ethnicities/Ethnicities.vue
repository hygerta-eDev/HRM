    ...
    <div class="container mx-auto p-6 bg-white shadow-xl rounded-lg border-blue-800" :class="{ 'blurred': deleteDialogVisible }">
      <div class="relative mb-6">
        <nav class="flex items-center text-sm text-gray-700">
          ...
          <span class="mx-2">></span>
          <span class="font-semibold">Ethnicities</span>
        </nav>
      </div>
        ...
        <div class="flex items-center">
          <router-link to="/Ethnicities/NewEthnicity">
            <button class="bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition duration-300 flex items-center shadow-md">
              <i class="fas fa-plus-circle fa-lg mr-2"></i>New Ethnicity
            </button>
          </router-link>
        </div>
        </div>
        ...
        <table class="min-w-full border-collapse">
          <thead>
            <tr class="bg-sky-600 text-white">
              ...
              <th class="px-4 py-4 text-base border-r border-gray-200 text-center font-semibold">Name</th>
              <th class="px-4 py-4 text-base text-center font-semibold">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(ethnicity, index) in filteredCompanies" :key="ethnicity.id"
              ...
              <td class="rounded-lg px-4 py-4 text-base border-r border-gray-200 text-center">{{ ethnicity.name }}</td>
              <td class="rounded-lg px-4 py-4 text-center">
                <div class="flex items-center justify-center h-full space-x-6">
                  ...
                  <i @click="confirmDelete(ethnicity.id, ethnicity.name)" class="fas fa-trash-alt fa-lg text-red-500 cursor-pointer hover:text-red-600 transition duration-300"></i>
                </div>
              </td>
            </tr>
          </tbody>
...
const filteredCompanies = computed(() => {
  if (searchQuery.value) {
    return Ethnicities.value.filter(ethnicity => 
      ethnicity.name.toLowerCase().includes(searchQuery.value.toLowerCase()) 
      // ethnicity.slug.toLowerCase().includes(searchQuery.value.toLowerCase())
    );
  }
  return paginatedEthnicities.value;
});
