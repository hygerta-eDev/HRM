<template>
    <div class="mt-10 p-4 sm:ml-64 text-black">    
        <div class="flex justify-between items-center mb-4 mt-5">
          ...
          <button @click="createJobPosition(department && department.id)" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Create New JobPosition</button>
        </div>
    </div>
    <hr>
    ...
    <div v-for="(department, index) in departmentsWithJobPositions" :key="index">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(jobPosition, jobIndex) in department.jobPositions" :key="jobIndex">
            <td class="px-6 py-4 whitespace-nowrap">{{ jobIndex + 1 }}</td>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              <button @click="editJobPosition(jobPosition)" type="submit" class="ml-5 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">EDIT</button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <script setup>
    ...
    const editJobPosition = (jobPosition) => {
        router.push(`/jobPositions/edit-jobPosition/${jobPosition.id}`);
    };
