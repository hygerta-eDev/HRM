<template>
    ...
    <main class="flex-1 p-8 sm:ml-64">
        <div class="mt-10 p-12 text-black">
            <div class="flex justify-between items-center mb-4">
                    <h2 class="text-2xl font-bold">Edit Ethnicity</h2>
            ...
            <form @submit.prevent="submitForm" class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4">                
                    <div class="flex-1">
                        ...
                        <input type="text" id="name" v-model="editedEthnicity.name" class="bg-gray-50 h-16 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
  ...
  <script setup>
    ...
    const editedEthnicity = ref({});
    const ethnicityId = router.currentRoute.value.params.id;
    ...
    onMounted(async () => {
        try {
        const response = await api.get(`/ethnicities/${ethnicityId}`);
        editedEthnicity.value = response.data;
    ...
    const submitForm = async () => {
        try {
        await api.put(`/ethnicities/update_ethnicity/${ethnicityId}`, editedEthnicity.value);
        router.push('/ethnicities'); // Redirect to departments list after editing

        } catch (error) {
        console.error('Error editing ethnicity:', error);
        }
    };
  </script>
