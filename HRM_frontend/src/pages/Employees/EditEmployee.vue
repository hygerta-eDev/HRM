<template>
    ...
    <button @click="downloadCV" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Download CV</button>
</div>
...
// employee.value.created_at = getCurrentDateTime(); 
        ...
        employee.value = response.data;
            ...
            employee.value.date_of_birth = formatDate(employee.value.date_of_birth);
        employee.value.date_hired = formatDate(employee.value.date_hired);
        employee.value.contract_end_date = formatDate(employee.value.contract_end_date);
    ...
    const downloadCV = async () => {
        try {
            // Check if the employee object has the cv_path property
            if (employee.value.cv) {
                // Make an API call to download the CV
                // Assuming the API endpoint returns the CV file
                const response = await api.get(`/download/cv/${employee.value.id}`, { responseType: 'blob' });
                ...
                link.href = url;
                link.download = 'employee_cv.pdf'; // Specify the filename here
                link.click();

                // Release the object URL to free up resources
                window.URL.revokeObjectURL(url);
            } else {
                // Display a message if the cv_path is not available
                console.error('CV path not found for the employee.');
            }
        } catch (error) {
    ...
    const submitForm = async () => {
        ...
        employee.value.update_at = getCurrentDateTime(); 
        await api.put(`/employess/update_employee/${employeeId}`, editedEmployee.value);
        router.push('/employees'); // Redirect to departments list after editing
        // router.emit('employeeUpdated', editedEmployee.value);
