...
const createQualification = async () => {
    try {
        const response = await api.post('/qualification/create_qualifications', qualification.value);
        router.push('/qualifications'); // Redirect to qualifications list after creating qualification
        console.log('Qualification created successfully:', response.data);
    } catch (error) {
        console.error('There was a problem creating the qualification:', error);
    }
};
</script>
    ...
    <main class="flex-1 p-8 sm:ml-64">
            <div class="mt-10 p-12 text-black">
                <div class="flex justify-between items-center mb-4">
                        <h2 class="text-2xl font-bold">Create Qualification</h2>
                ...
                <form @submit.prevent="createQualification" class="space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4">                
                        <div class="flex-1">
                            <label for="name" class="block mb-2  text-sm font-medium text-black dark:text-black">Name:</label>
                            <input type="text" id="name" v-model="qualification.name" class="bg-gray-50 h-16 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        </div>
                        <div class="flex-1">
                            <label for="slug" class="block mb-2 text-sm font-medium text-black dark:text-black">Description:</label>
                            <textarea id="slug" v-model="qualification.slug" class="bg-gray-50  border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
                        </div>
